...
public class EditStallmanager extends BaseActivity implements
	...
	protected void getRole() {
		...
		/*
		 * if(ispigou){ roles=roles+"1,"; } else if(isdaigou){ roles=roles+"2,";
		 * } else if(ismanagerzhongduan){ roles=roles+"3,"; } else if(isquary){
		 * roles=roles+"4,"; } else if(ismanageragent){ roles=roles+"5,"; } else
		 * if(ismanageruser){ roles=roles+"6,"; } else if(ismanagerworker){
		 * roles=roles+"7,"; } else if(ismanagerAddres){ roles=roles+"8,"; }
		 */
	}
	...
	protected void editStaffff() {
		...
		Config.editStaff(EditStallmanager.this,
				...
				rolesnew, et_paw.getText().toString(), new HttpCallback(
						EditStallmanager.this) {
