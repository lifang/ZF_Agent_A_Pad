...
public class Mwdxx extends Fragment implements IXListViewListener,
	...
	private void getData() {
		Config.getMsgList(getActivity(), agentUserId, page, rows,
				new HttpCallback<Page<MessageEntity>>(getActivity()) {
	...
	private void isRead() {
		/*
		 * for(int i=0;i<datamsg.size();i++){ if(datamsg.get(i).getIscheck()){
		 * ids[size]=String.valueOf(datamsg.get(i).getId()); size++; } }
		 */
		...
		Config.IsRead(getActivity(), ids, agentUserId, new HttpCallback(
				getActivity()) {
	...
	private void delectMore() {
		/*
		 * ids=new String[list.size()]; for(int i=0;i<list.size();i++){
		...
		Config.DelectMoreMsg(getActivity(), ids, agentUserId, new HttpCallback(
				getActivity()) {
